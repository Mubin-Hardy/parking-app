...
def vehicleentry(request):
    ...
    if request.method == "POST":
        ...
        venter = Vehicleentry(vnumber=vnumber,vtype=vtype,contactno=contactno,date=date,
                              ...
                              tagno=tagno,advancepayment=apay)
        venter.save()
...
def vehicleexit(request):
    ...
    if request.method == 'POST':
        ...
        user = Vehicleentry.objects.filter(vnumber=vno,vtype=vty,tagno=tno,advancepayment=ap)
        if len(user)>0:
            if vty == 'two':
                if tdiff <= tc1:
                    farem = 15
            ...
            fpay = farem - up
            pay = Vehicleentry()
            print(pay.advancepayment)
            print(fpay)
            exitt = Vehicleexit(vno=vno,vty=vty,outtime=outtime,farem=farem,tno=tno,finalpay = fpay)
            exitt.save()
